...
public class MainActivity extends Activity {
    ...
    //計算するメソッド
    private static final int PLUS = R.id.plus;
    private static final int MINAS = R.id.minus;
    private static final int MULTIPLY = R.id.multiply;
    private static final int DIVIDE = R.id.divide;
    double calc(int operator, double value1, double value2) {
        double ans=0;
        if(operator == PLUS){
            ans = value1 + value2;
        } else if (operator == MINAS) {
            ans = value1 - value2;
